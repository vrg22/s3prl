...
class Runner():
    ...
    def train(self):
        ...
        while pbar.n < pbar.total:
            ...
            for batch_id, (wavs, comp_wavs,  *others) in enumerate(tqdm(dataloader, dynamic_ncols=True, desc='train', file=tqdm_file)):
                ...
                try:
                    ...
                    comp_wavs = [torch.FloatTensor(wav).to(self.args.device) for wav in comp_wavs]

                    if self.upstream.trainable:
                        features = self.upstream.model(wavs)
                        comp_features = self.upstream.model(comp_wavs)
                    else:
                        with torch.no_grad():
                            features = self.upstream.model(wavs)
                            comp_features = self.upstream.model(comp_wavs)
                    features = self.featurizer.model(wavs, features)
                    comp_features = self.featurizer.model(comp_wavs, comp_features)

                    if specaug:
                        features, _ = specaug(features)
                        comp_features, _ = specaug(comp_features)
